...
( function( $ ) {
  var OptionPrefix  = TCGCPreviewParams.OptionPrefix,
      $_figures     = $('figure, .tc-grid-figure', '.tc-gc'),
      api           = wp.customize;
  ...
  api( OptionPrefix + '[tc_gc_title_color]' , function( value ) {
    value.bind( function( to ) {
      ...
      if ( 'custom' == to )
        $( '.tc-gc .entry-title a').css('color', api( 'tc_theme_options[tc_gc_title_custom_color]' ).get() );
      else
        $( '.tc-gc .entry-title a').css('color', '' );
    } );
  } );

  api( OptionPrefix +'[tc_gc_title_custom_color]' , function( value ) {
    value.bind( function( to ) {
      $('.gc-custom-title .entry-title a').css('color', to ).addClass('previewing');
    } );
  } );
  ...
  api( OptionPrefix + '[tc_gc_effect]' , function( value ) {
    value.bind( function( to ) {
      var _new_effect = to,
          _vertical_align_class = 'czr-talign';
      ...
      $_figures.each( function(){
        var $_this          = $(this),
            _current_effect = $_this.attr('class').match(/effect-[0-9]+/)[0];

        $_this.addClass(_new_effect).removeClass(_current_effect);

        //remove vertical align class if not expanded for c4 only
        //and add it if new effect matches one of the list
        if ( typeof undefined !== typeof TCGCPreviewParams.VSummaryCenteredEffects && $_this.closest( '.gc-expanderd' ).length < 1 ) {
          $_this.find( '.entry-summary' ).removeClass( _vertical_align_class );
          if ( _.contains( TCGCPreviewParams.VSummaryCenteredEffects, _new_effect ) ) {
            $_this.find( '.entry-summary' ).addClass( _vertical_align_class );
          }
        }

        if ( 'effect-4' == _current_effect || 'effect-4' == _new_effect ){
          var _excerpt_content = $_this.find('.tc-g-cont'),
